...
## 多様性（厳守）
- ★評価が高いものや有名なものを自動的に優先しないこと
- 候補の中から「2〜3番目の選択肢」を意識的に選ぶこと
- 異なる呼び出しでは必ず異なるスポットを選ぶこと。同じ条件でも前回と別の組み合わせを試すこと
- ユニークな体験・穴場・地元民に人気の場所を積極的に採用すること
...
毎回異なる選択をすること。有名・定番スポットより個性的な場所を優先してください。
出力はJSON: { "selectedAliases": ["X1", "Y2", "Z3"] }

今回の探索ID: ${Math.random().toString(36).slice(2, 8)}`;
}
